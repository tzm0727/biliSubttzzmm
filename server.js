...
async function fetchYoutubeSubtitle(rawUrl) {
  ...
  let primaryErr = null;
  try {
    const data = await fetchYtSubtitleAndroid(rawUrl, videoId);
  ...
  try {
    const data = await fetchYtSubtitleLegacy(rawUrl, videoId);
    ytCache.set(videoId, { at: Date.now(), data });
    return data;
  } catch (e) {
    throw primaryErr || e;
...
async function handleApi(req, res, url, body) {
  ...
  if (req.method === "GET" && url.pathname === "/api/youtube/fetch") {
    ...
    try {
      ...
      if (/not available.*language|language.*not available/i.test(msg))
        msg = "该视频没有所选语言的字幕";
