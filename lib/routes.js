...
async function handleApi(req, res, url, body) {
  ...
  if (req.method === "POST" && url.pathname === "/api/export/epub") {
    ...
    if (!content) return sendJson(res, 400, { error: "缺少内容" });
    const epub = buildEpub({ title, content });
    const filename = encodeURIComponent(title) + ".epub";
    res.writeHead(200, {
  ...
  if (req.method === "GET" && url.pathname === "/api/automation/status") {
    return sendJson(res, 200, Object.assign({ ok: true }, automation.status()));
  }

  if (req.method === "POST" && url.pathname === "/api/automation/sync") {
    const nextRuns = automation.syncRules(body.automations || []);
    return sendJson(res, 200, { ok: true, count: nextRuns.length, nextRuns });
  }

  if (req.method === "POST" && url.pathname === "/api/automation/run") {
    const rule = body.automation;
    if (!rule || !rule.name) {
      return sendJson(res, 400, { error: "缺少自动化规则" });
    }
    try {
      const report = await automation.executeRule(rule, {
        apiKey: String(body.apiKey || "").trim(),
        model: String(body.model || "deepseek-chat"),
        dataText: String(body.dataText || ""),
      });
  ...
  if (req.method === "GET" && url.pathname === "/api/automation/reports") {
    return sendJson(res, 200, { ok: true, reports: automation.listReportsAPI() });
  }

  if (req.method === "POST" && url.pathname === "/api/automation/clear-reports") {
    automation.clearReports();
